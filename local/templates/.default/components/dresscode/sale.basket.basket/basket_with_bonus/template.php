...
<?
	$templates_directory = $arResult['BONUS_PRODUCTS_IN_BASKET'] ? "/bonus_products" : "/catalog_products";
	
	if ($arResult['BONUS_PRODUCTS_IN_BASKET']) { ?>
		<script>
		// çàìåíÿåì ðóá. íà áàëë.
		$(document).ready(function() {
			$(document).ajaxComplete(function() {
				$("#basketProductList .points_item .price, #basketProductList .points_item .sum").each(function(){
					$(this).html($(this).html().replace("ðóá.", "áàëë."));
				});
			});
		});
		</script>
	...
	$arBasketTemplates = array(
		"SQUARES" => array(
			...
			"TEMPLATE_FILE" => "/include" . $templates_directory . "/basket_squares.php",
			"CLASS_NAME" => "squares"
		),
		"TABLE" => array(
			"CHANGE_URL" => $APPLICATION->GetCurPageParam("basketView=table", array("basketView")),
			"TEMPLATE_FILE" => "/include" . $templates_directory . "/basket_table.php",
			"CLASS_NAME" => "table"
		)
	);
	...
	<div id="personalCart">
		...
		<div class="orderLine">
			<div id="sum">
				...
				<span class="price">
					<? if ($arResult['BONUS_PRODUCTS_IN_BASKET']) { ?>
						<span id="allSum"><?= $arResult["SUM"] . " " . GetMessage("POINTS") ?></span>
					<? } else {?>
