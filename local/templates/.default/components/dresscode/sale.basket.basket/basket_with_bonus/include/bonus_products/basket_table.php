<table class="productTable">
	...
	<tbody>
		...
		<?$countPos += $arElement["QUANTITY"] ?>
			<tr class="basketItemsRow points_item parent" data-id="<?=$arElement["ID"]?>" data-cart-id="<?=$arElement["ID"]?>"> 
				<td><span class="pic"><img src="<?=!empty($arElement["INFO"]["PICTURE"]["src"]) ? $arElement["INFO"]["PICTURE"]["src"] : SITE_TEMPLATE_PATH."/images/empty.png"?>" alt="<?=$arElement["INFO"]["NAME"]?>"></span></td>
				<td class="name"><span><?=$arElement["INFO"]["NAME"]?></span></td>
				<td class="bQty">		
					<div class="basketQty">
				...
				<td>
					<span class="price">		      
						...
						<?endif;?>
  						<?= $arElement["BASE_PRICE"] . " " . GetMessage("POINTS_SHORT") ?> 
  					</span>
  				</td>
  				<td>
  					<span class="sum" data-price="<?=doubleval($arElement["PRICE"])?>"><?= ($arElement["BASE_PRICE"] * doubleval($arElement["QUANTITY"])) . " " . GetMessage("POINTS_SHORT") ?> </span>
  				</td>
