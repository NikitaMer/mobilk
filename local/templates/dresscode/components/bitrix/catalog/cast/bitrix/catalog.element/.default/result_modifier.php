<?
	...
	$OPTION_QUANTITY_TRACE = COption::GetOptionString("catalog", "quantity_trace");
	...
	$arResult["GIFT_PRICE_CODE"] = array();
	...
	if(!empty($arParams["PRICE_CODE"])){
		...
		while ($arPriceType = $dbPriceType->Fetch()){
			if($arPriceType["CAN_BUY"] == "Y"){
		    	...
		    	$arResult["GIFT_PRICE_CODE"][] = $arPriceType["NAME"];
			}
	...
	if(!empty($arResult["PRODUCT_PRICE_ALLOW"]) && !empty($arPriceCodes) || empty($arParams["PRICE_CODE"])){
		$arResult["TMP_PRICE"] = CCatalogProduct::GetOptimalPrice($arResult["ID"], 1, $USER->GetUserGroupArray(), "N", $arPriceCodes);
	...
	if($arResult["SKU"]){
		...
		if (is_array($SKU_INFO)){
			...
			$rsOffers = CIBlockElement::GetList(array(),$arOffersFilter , false, false, array("ID", "IBLOCK_ID", "DETAIL_PAGE_URL", "DETAIL_PICTURE", "NAME", "CATALOG_QUANTITY", "CATALOG_AVAILABLE", "CATALOG_QUANTITY_TRACE", "CATALOG_CAN_BUY_ZERO"));
	...
	if(!empty($arResult["SKU_PRODUCT"]) && !empty($arResult["PROPERTIES_LIST"])){
		...
		foreach ($arResult["SKU_PRODUCT"] as $ir => $arOffer) {
			...
			if($active){
				...
				$arResult["CATALOG_QUANTITY_TRACE"] = $arOffer["CATALOG_QUANTITY_TRACE"];
				$arResult["CATALOG_CAN_BUY_ZERO"] = $arOffer["CATALOG_CAN_BUY_ZERO"];
