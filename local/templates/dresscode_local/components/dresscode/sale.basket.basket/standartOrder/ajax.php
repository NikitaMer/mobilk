		...
		if(!empty($_GET["LOCATION_ID"])){
			...
			while ($arRes = $dbRes->Fetch()){
				if($arRes["PAYSYSTEM_ID"] == $_GET["PAYSYSTEM_ID"]){
					$arRes["DELIVERY_ID"] = \Bitrix\Sale\Delivery\Services\Table::getIdByCode($arRes["DELIVERY_ID"]);
			...
			foreach($arDeliveryServiceAll as $deliveryObj) 
			{
				...
				if (count($arP2D[$arUserResult["PAY_SYSTEM_ID"]]) <= 0 || in_array($deliveryObj->id, $arP2D[$arUserResult["PAY_SYSTEM_ID"]]) || in_array($deliveryObj->parentId, $arP2D[$arUserResult["PAY_SYSTEM_ID"]]))
				{
					
					if(in_array($deliveryObj->parentId, $arP2D[$arUserResult["PAY_SYSTEM_ID"]])){
						$arP2D[$deliveryObj->id] = $arP2D[$deliveryObj->parentId];
					}
					...
					$arDelivery = array(
						...
						$arDelivery["PRICE_FORMATED"] = str_replace("-", ".", strip_tags(SaleFormatCurrency($calcResult->getPrice(), $OPTION_CURRENCY)));
						$arDelivery["CURRENCY"] = $OPTION_CURRENCY;
						$arDelivery["PRICE"] = ceil($calcResult->getPrice());
			...
			foreach($arDeliveryServiceAll as $deliveryObj) 
			{
				if (count($arP2D[$arUserResult["PAY_SYSTEM_ID"]]) <= 0 || in_array($deliveryObj->id, $arP2D[$arUserResult["PAY_SYSTEM_ID"]]) || in_array($deliveryObj->parentId, $arP2D[$arUserResult["PAY_SYSTEM_ID"]]))
				{
					...
					$arDelivery = array(
						...
						$arDelivery["PRICE_FORMATED"] = SaleFormatCurrency($calcResult->getPrice(), $OPTION_CURRENCY);
						$arDelivery["CURRENCY"] = $OPTION_CURRENCY;
