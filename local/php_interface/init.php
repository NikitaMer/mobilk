...
/**
 * 
 * ѕровер€ем, принадлежит ли пользователь к группе продавцов
 * 
 * @return bool
 * */
...
/**
 * 
 * ѕровер€ем, имеет ли пользователь статус серебр€ного партнера
 * 
 * @param int $user_id
 * @return bool
 * */
...
/**
 * 
 * ѕровер€ем, имеет ли пользователь статус золотого партнера
 * 
 * @param int $user_id
 * @return bool
 * */
...
/**
 * 
 * ѕровер€ем, есть ли у пользовател€ активные за€вки на продавца
 * 
 * @return bool
 * */
...
/**
 * 
 * ѕолучить список групп пользовател€
 * 
 * @param int $user_id
 * @return array $groups
 * */
...
/**
 * 
 * ѕровер€ем, €вл€етс€ ли элемент бонусным товаром
 * 
 * @param int $iblock_id
 * @return bool
 * */
...
/**
 * 
 * ѕолучить количество баллов у пользовател€
 * 
 * @param int $user_id
 * @return int $balance
 * */
...
/**
 * 
 * ѕолучить общее количество баллов у пользовател€ за все врем€
 * 
 * @param int $user_id
...
function getBasketType() {
	// получаем товары в корзине дл€ текущего пользовател€
	$type = "empty";
	$products_ids = array();
	...
	if ($basket_products->SelectedRowsCount()) {
		...
		// т.к. у товаров в корзине нет id инфоблока, то получим его дл€ всех товаров в корзине
		$iblock_ids = array();
...
/**
 * 
 * ѕолучить кол-во баллов за товар с учетом группы пользовател€
 * 
...
/**
 * 
 * ƒобавить баллы на внутренний счет пользовател€
 * 
 * @param int $user_id
 * @param int $points
 * @return void
...
/**
 * 
 * ƒобавить баллы на общий счет пользовател€
 * 
 * @param int $user_id
 * @param int $points
 * @return void
...
function updateUserTotalPoints($user_id, $points) {
	// получаем текущее кол-во баллов пользовател€
	$current_points = getTotalUserPoints($user_id);
	// обновл€ем общие баллы пользовател€
...
/**
 * 
 * ¬озвращаем границы дл€ уровней продавцов
 * 
 * @return array $result
 * 
 * */
...
/**
 * 
 * ѕровер€ем уровень пользовател€ и, если баллов достаточно, то перекидываем его дальше
 * 
 * @param int $user_id
 * @return void
 * 
...
function checkUserLvl($user_id) {
	...
	if ($user_have_golden_status && $total < $borders[GOLDEN_LVL_ID]['from']) {
		// удал€ем пользовател€ из золотых партнеров
	...
	if (!empty($groups_ids)) {
		// если есть что записать в список групп, то обновл€ем
		$user = new CUser;
		$fields = Array(
			"GROUP_ID"=> $groups_ids
		);
